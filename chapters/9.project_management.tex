...
The project also carried a consistent discipline of verifying a decision or a piece of code before treating it as final. The scoring engine's own test suite, seventy tests at the point of integration, was kept green throughout its move into the backend, and an end-to-end script, \texttt{score\_test.py}, cross-checks the full database-to-engine path against the engine called directly, to catch any distortion introduced by the translation layer between the two. The engine itself was also subjected to an audit carried out independently of its implementation, discussed in more detail in Chapter~\ref{chap:validation}. What belongs here is the discipline it reflects: treating a component as trustworthy only once it had been checked by something other than the reasoning that produced it in the first place.
...
The underlying cause combined two things. The session cookie carried \texttt{SameSite=Lax} by design, a deliberate security choice, but the frontend and backend ran on different ports (\texttt{localhost:3000} and \texttt{localhost:8000}), which the browser treated as cross-site and refused to store the cookie against. A second, hidden factor made the bug harder to see: an unversioned, VM-specific \texttt{NEXT\_\allowbreak PUBLIC\_\allowbreak API\_\allowbreak URL} entry sitting in a local \texttt{.env.local} file forced every API call onto an absolute cross-origin URL, which is precisely why the bug only ever appeared on the VM and never on my own machine. The fix, a Next.js proxy relaying \texttt{/api/*} to the backend so the browser only ever sees one origin, did not touch the cookie's security attributes at all.
...
The choice of \texttt{qwen2.5:3b} as the deployed model, discussed from the design side in Chapter~\ref{chap:method} and revisited empirically in Chapter~\ref{chap:validation}, was in practice an arbitration made under a hard constraint rather than a considered comparison. The demo ran on CPU inside a VM, with no GPU available, and a 7B-class model such as \texttt{mistral:7b-instruct} pushed generation time into several minutes per request, which made the recommendations endpoint impractical to use or to demonstrate. The 3B model kept generation to roughly a minute, still slow but usable.

The honest limitation is that this was a latency-driven decision made against one data point, not a benchmark. None of the larger candidates listed at the start of the project, \texttt{mistral:7b-instruct}, \texttt{mixtral:8x7b}, \texttt{llama3.1:8b}, \texttt{qwen2.5:7b}, was ever actually measured on the project's hardware, only \texttt{qwen2.5:3b}'s generation time was. I stand by the choice given the constraint, a model nobody can wait for is not a useful demo regardless of how much better its output might be, but I would not present it as a rigorously benchmarked trade-off, and Chapter~\ref{chap:validation} is explicit about the same limitation.

\subsection{Reliability of a small local model}
\label{sec:pm-model-reliability}

A 3B-parameter model is a small model, and treating its output as reliable enough to show a user required accepting that it would sometimes be wrong, then designing around that rather than assuming it away. The task it was given is narrow by design, summarising and enriching a short list of already-computed, weak best practices rather than reasoning openly over the assessment, which is precisely what makes a small model viable here at all. Even so, the guardrails described in Chapter~\ref{chap:platform}, anchoring generation to the listed best-practice identifiers, discarding any recommendation that cites one outside the referential, and falling back to the referential's static recommendations on any failure, exist because I could not treat the model's output as trustworthy by default. The lesson I take from this is less about the specific model and more about the posture: a local, small, non-deterministic component can still be shipped responsibly, but only if the surrounding system is built to assume it will occasionally misbehave, not as an afterthought bolted on once it did.

\subsection{A broken dependency: \texttt{passlib} versus \texttt{bcrypt}}
\label{sec:pm-passlib}

A smaller but telling incident: the plan was to hash passwords through \texttt{passlib}, a well-known abstraction over several hashing backends, but it turned out to be unmaintained and to crash outright against a current \texttt{bcrypt} release, because it inspects a version attribute that no longer exists on the newer package. The fix was to call \texttt{bcrypt} directly instead, which turned out to be fewer moving parts for the same guarantee. The incident cost little time to fix once diagnosed, but it is a small, concrete example of a risk that a completely unlocked dependency set carries throughout a project, taken up more generally in Section~\ref{sec:pm-differently}.
...
The \texttt{passlib} incident described in Section~\ref{sec:pm-passlib} would not have been avoided by a lockfile. It was a compatibility failure at the root, an unmaintained library inspecting an attribute that a current \texttt{bcrypt} release no longer exposes, and that wall appears whenever the two are paired, locked or not. A lockfile could only have postponed the discovery by pinning an older, compatible \texttt{bcrypt}, which trades one problem for another rather than solving anything. The actual lesson from that incident is about which dependencies to pick, not about how strictly their versions are pinned.
...
Section~\ref{sec:pm-model-arbitration} already owns up to the fact that the choice of \texttt{qwen2.5:3b} was never checked against the larger candidates on the project's own hardware, only reasoned about from their published sizes and the CPU constraint. If I were to redo this part of the project, I would put a small, even rough, benchmark of two or three model sizes on the actual target hardware early, before committing to one, rather than treating the decision as settled after the first model that produced usable output in acceptable time. It would not have changed the outcome, a CPU-only VM was never going to run a 7B model comfortably, but it would have turned a defensible guess into a documented comparison, which matters for a report whose whole premise is that choices should be traceable and justified rather than merely reasonable in hindsight.
...
\subsection{Other honest points}
\label{sec:pm-other-points}

% TODO: en attente, on y revient plus tard.
