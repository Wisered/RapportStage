% =====================================================================
%  The Scoring Engine: Methodology and Implementation
%  PFE — Arthur Saunier — TELECOM Nancy / Wavestone Luxembourg
%  >>> Version adaptee a la classe « tnreport » de TELECOM Nancy <<<
%
%  ---------------------------------------------------------------------
%  1) PREAMBULE : RIEN A AJOUTER.
%  ---------------------------------------------------------------------
%  La classe tnreport charge deja tout ce que ce chapitre utilise :
%  booktabs, tikz + \usetikzlibrary{positioning, arrows.meta, fit},
%  listings (avec son \lstset), xcolor, cleveref et hyperref. Le chapitre
%  compile tel quel, sans toucher au preambule. (cleveref est disponible :
%  \cref fonctionne si tu veux, mais la prose utilise \ref.)
%
%  ---------------------------------------------------------------------
%  2) INSERER LE CHAPITRE : sauver ce fichier sous chapters/scoring_engine.tex,
%     puis, a l'endroit voulu dans Rapport_Stage.tex (entre deux chapitres,
%     avant la Conclusion) :
%         \input{chapters/scoring_engine}
%     (ou coller directement le corps ci-dessous). Il apparaitra
%     automatiquement dans la table des matieres, la liste des figures /
%     tableaux et le \lstlistoflistings de la classe.
%
%  ---------------------------------------------------------------------
%  3) LABELS D'AUTRES CHAPITRES REFERENCES ICI (a faire correspondre aux
%     tiens, sinon les renvois resteront « undefined ») :
%  ---------------------------------------------------------------------
%    chap:method    -> ton chapitre « Methodological Approach »
%    chap:platform  -> ton chapitre « Platform Architecture »
%    chap:pm        -> ton chapitre « Project Management »
%
%  NB : le listing de 5.3 est ta vraie fonction engine.py (imports et
%  docstring omis pour la place). Labels internes : sec:se-*, fig:se-*,
%  tab:se-*, lst:se-*.
% =====================================================================
...
% ---------------------------------------------------------------------
%  CHAPEAU DU CHAPITRE (~½ page)
%  Objectif : poser LE problème d'ingénierie avant toute solution.
%  Fil directeur à faire ressortir :
%    « Comment transformer une auto-évaluation déclarative et subjective
%      de maturité de gestion de crise cyber en un score qu'un consultant
%      peut défendre, chiffre par chiffre, devant un client — c.-à-d.
%      déterministe, auditable et résistant au gaming ? »
%  Points à couvrir :
%    - pourquoi une moyenne pondérée naïve échoue : opaque, manipulable,
%      non reproductible ;
%    - énoncer les 2 propriétés non négociables : AUDITABILITÉ + ANTI-DÉCLARATIF ;
%    - annoncer que chaque mécanisme du moteur répond à un mode de défaillance ;
%    - annoncer le plan des 5 sections.
% ---------------------------------------------------------------------
...
\label{sec:se-spec}
% ---------------------------------------------------------------------
%  RÔLE : le « cahier des charges » du moteur. Section analyse du besoin.
%  Critères tuteur servis : choix justifiés (dérivation besoin -> spec).
%
%  À couvrir :
%   - De la douleur métier à la spécification : partir du besoin consulting
%     (défendre un chiffre devant le client, empêcher l'auto-diagnostic
%     complaisant) et EN DÉRIVER les exigences formelles (ne pas présenter
%     la spec comme donnée).
%   - Objets du domaine : 4 phases (Preparedness / Response / Recovery / ...),
%     16 best practices, 48 questions (3 par BP), échelle de maturité,
%     types de preuve, paires de cohérence, règles N/A.
%   - Le référentiel 1.1.0 comme SOURCE DE VÉRITÉ UNIQUE.
%   - Le contrat de sortie : PAS un score mais un AssessmentResult
%     (gates_applied, penalties_applied, cap_applied/question,
%      capped_question_ids, N/A) -> pose l'exigence d'auditabilité.
%   - Exigences dérivées : déterminisme, reproductibilité, pilotage par le
%     référentiel, anti-gaming (chacune sera « payée » en 5.2 / 5.3).
% ---------------------------------------------------------------------

The scoring specification has a concrete origin. The ENISA framework~\cite{enisa2024}
describes sixteen best practices across four crisis phases, but it stops at
description: it offers no way to measure how mature an organisation actually is
against them. At the same time, the NIS2 directive~\cite{nis2} pushes organisations
...
ratings inspired by the NIST Cybersecurity Framework~\cite{nistcsf}, do not map
...
every threshold the engine uses. Table~\ref{tab:se-referential} summarises its
...
global score. Every phase groups several best practices (BPs), and every best
...
CMMI-style five-point scale~\cite{cmmi}, from 0 (\emph{Not in place}) to 4
...
\emph{Not Applicable} when it genuinely does not fit the organisation, within the
strict limits detailed in Section~\ref{sec:se-pipeline}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llcc}
    ...
    \midrule
    % TODO : compléter avec les vraies valeurs du référentiel
    Prevention   & 3  & 9  & 25\,\% \\
...
% ---------------------------------------------------------------------
%  RÔLE : cœur intellectuel du chapitre. Loger ici « problématique +
%  solution intelligente ». Présenter le calcul comme un pipeline en
%  ORDRE FIXE, chaque étape = réponse à un mode de défaillance.
%
%  Étapes (ordre fixe) :
%    1. Agrégation de la maturité brute par question.
%    2. Caps de preuve / question  -> anti-déclaratif (niveau 4 sans preuve
%       formelle plafonné à 1 ; caps monotones no_formal_evidence->1 ...
%       real_incident->4).
%    3. Agrégation au niveau BP.
%    4. Gates : un BP foundational sous son seuil plafonne la phase/le global.
%       Règle « le cap le plus strict gagne » entre gates multiples.
%    5. Pénalités de cohérence : paires de BP appariés divergeant de > 2
%       niveaux -> sanction forfaitaire (DIFFÉRENCIATEUR MÉTHODOLOGIQUE, ta
%       contribution originale).
%    6. Normalisation des phases à 25 % (évite que Preparedness (9 BP)
%       écrase Response/Recovery (2 BP)).
%    7. Finalisation -> mapping vers le niveau global.
%
%  À mettre en valeur (signaux « ingénieur ») :
%    - POURQUOI l'ordre est fixe : ordre gate -> pénalité justifié par
%      l'exemple chiffré (60 vs 65, cf. CLAUDE.md §6.1). Reprendre tel quel.
%    - Neutralité du N/A : exclu du numérateur ET du dénominateur (pas un 0).
%
%  RECOMMANDÉ : (a) schéma du pipeline en figure ; (b) exemple numérique
%  déroulé de bout en bout dans un tableau.
% ---------------------------------------------------------------------
...
result. Consider an assessment whose raw global score is 72, where a foundational
gate caps it at 65 and one consistency pair triggers a five-point penalty. Applying
the gate first gives 65, then the penalty brings it to 60. Reversing the order would
...
% -- Emplacement figure : schéma du pipeline en ordre fixe
\begin{figure}[htbp]
  \centering
...
% -- Emplacement tableau : exemple numérique déroulé de bout en bout
\begin{table}[htbp]
  \centering
...
% ---------------------------------------------------------------------
%  RÔLE : traduction de la spec en code défendable. Section conception.
%  Critères tuteur : choix justifiés (architecture), qualité d'ingénierie.
%
%  À couvrir :
%   - Les 5 principes inviolables (P1–P5), surtout :
%       P1 : moteur pur, synchrone, sans I/O ni hasard -> déterminisme ;
%       P4 : référentiel = source de vérité unique, un seul loader.
%   - Décomposition en modules : referential_loader, evidence, na_rules,
%     aggregation, finalization, engine + models/ (referential = contrat,
%     inputs = brut utilisateur, outputs = trace d'audit). Une fonction pure
%     et courte par étape -> le code se lit comme la spec de 5.2.
%   - Design défensif : immutabilité (frozen=True, extra="forbid"),
%     validation croisée au chargement (3 questions/BP, contiguïté 0->100
%     des niveaux, foundational_cannot_be_na), bornes garanties par
%     construction ET par filet Pydantic (Field(ge=0, le=100)).
%   - Anticipation de l'intégration (pont vers le Ch6, \cref{chap:platform}) :
%     moteur agnostique de la DB, traduction DB->moteur isolée dans un seul
%     « bridge », score recalculé à la volée et jamais stocké (P1).
% ---------------------------------------------------------------------
...
Listing~\ref{lst:se-engine} shows that routine, and
Figure~\ref{fig:se-modules} shows the decomposition.
...
\begin{lstlisting}[language=Python,caption={The top-level \texttt{score\_assessment} routine: the code reads like the ordered pipeline of Section~\ref{sec:se-pipeline}.},label={lst:se-engine}]
...
% ---------------------------------------------------------------------
%  RÔLE : LA section la plus payante pour le tuteur.
%  Critères tuteur : problèmes rencontrés + résolution méthodique, recul
%  critique, validation southbound & northbound, gestion des risques.
%
%  À couvrir :
%   - Stratégie de test : tests par étage, scénarios « golden » comme spec
%     exécutable, test_determinism (verrouille P1), balayage paramétré des
%     bornes [0,100], cas d'erreur du loader.
%   - L'audit indépendant (16,5/20) et ses 2 angles morts, présentés comme
%     des PROBLÈMES RENCONTRÉS :
%       H1 : la branche des pénalités de cohérence (ton différenciateur)
%            n'était couverte par AUCUN test ;
%       H2 : penalty_percent_of_global_score appliqué comme points absolus,
%            le nom contredisait le code ;
%       + champs décoratifs (global_weight, foundational_can_be_na).
%   - La boucle de correction (CHANGELOG) : filet de sécurité AVANT refactor,
%     +3 tests (64->67), renommage -> penalty_points, décision tranchée
%     (points absolus assumés), bump du référentiel -> 1.1.0, 70 tests au total.
% ---------------------------------------------------------------------
...
and the documentation were put through an independent, read-only audit. It rated the
work 16.5 out of 20, and, more usefully, it surfaced two real blind spots that a set
of green tests had kept out of sight.
...
% -- Emplacement tableau : synthèse des findings d'audit et corrections
\begin{table}[htbp]
  \centering
...
% ---------------------------------------------------------------------
%  RÔLE : recul critique explicite (prépare le Ch9, \cref{chap:pm}).
%
%  À couvrir :
%   - Limites assumées + défenses :
%       * seuils « magiques » (min 40, caps 60/60/65/70, gap max 2,
%         pénalité 5) -> paramètres experts Wavestone, versionnés ;
%         idéalement esquisser une analyse de sensibilité ;
%       * pénalité fondée sur la maturité EFFECTIVE (post-cap) -> volontaire :
%         on mesure le démontrable, pas le déclaré ;
%       * normalisation 25 % comme constante structurelle ;
%       * arrondi bancaire aux frontières de niveau.
%   - Ce que je referais autrement -> transition vers le Ch9.
%   - Positionnement : déterminisme + auditabilité rendent l'étage IA (Ch6)
%     SÛR (l'IA ne voit que AssessmentResult + référentiel, jamais de PII
%     ni de justifications N/A).
% ---------------------------------------------------------------------
...
A few smaller decisions are of the same kind. The four phases carry equal weight, a
structural choice that stops Preparedness and its nine best practices from drowning
out the two-practice phases; that weighting is now read from the referential, so a
later version could revisit it. The penalty threshold is strict, so a gap of exactly
...
phrase recommendations for the weakest best practices, it is given only scores,
best-practice identifiers and referential text, never client names or the private
N/A justifications, and any failure on its side degrades to a deterministic
recommendation drawn from the referential. The engine remains the single arbiter of
