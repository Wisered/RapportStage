...
\subsection{An order of magnitude, stated as a line of reasoning}
\label{sec:impact-consulting-economics}

None of the value described so far has been expressed in money, and this subsection does not close that gap with a figure, because no measurement made in this work would support one. What can be set out is the chain of reasoning that connects the platform to an economic effect, together with the point at which that chain stops resting on evidence and starts resting on inference.

The external end of the chain is documented. As Section~\ref{sec:crisis-landscape} reports, what a breach costs varies considerably with how quickly it is identified and contained, by more than a million dollars on average between breaches resolved within two hundred days and those that run past them. The internal end is the platform, which acts on none of that directly: it detects nothing, contains nothing, and shortens no incident. What it produces is a structured picture of how prepared an organisation is across the sixteen best practices, including the four foundational ones whose weakness caps the whole score (Chapter~\ref{chap:scoring-engine}), together with a prioritised sense of where that preparation is thinnest.

Joining those two ends is an inference, and it is worth stating plainly rather than dressing it up as a result. An organisation that knows which parts of its crisis-management capability are weakest is better placed to strengthen them than one working from a general sense of exposure, and stronger capability is what the external figures associate with a less costly crisis. Every step in that chain is plausible; none of it has been tested here. No before-and-after comparison exists, no assessment has been run on an organisation that subsequently handled a real crisis, and this work is in no position to attach a percentage or an amount to the platform's effect on the cost of an incident. The economic case rests on the platform acting on a variable that external evidence links to cost. Whether it moves that variable, and by how much, is unobserved.
...
\subsection{Handover and status at the end of the internship}
\label{sec:impact-perspectives-handover}

Whatever becomes of the directions above, the deliverable itself has a concrete destination, and it is worth stating plainly at the close of this chapter. The work lives in a local git repository, handed over to Wavestone at the end of the internship together with the documentation written alongside it: the referential itself, the dated architecture and decision logs described in Chapter~\ref{chap:pm}, the scoring engine's audit and its changelog, and the notes covering the network setup the platform runs on.

What that hands over is a documented, working codebase. Someone picking it up would find the platform and its accompanying documents in a state that explains what was built and why, with one uneven part: the scoring engine, whose foundational gates, consistency penalties and fixed pipeline order have to be read carefully in Chapter~\ref{chap:scoring-engine} and in the referential itself before the reasoning behind any given score becomes clear. The rest is straightforward to follow from the documentation as it stands. Beyond the handover, nothing is settled, and Section~\ref{sec:impact-consulting} has already recorded that no decision has been made about whether Wavestone will take the tool further. The honest description of its status at the end of the internship is therefore a complete and documented codebase in the practice's hands, available as a starting point for the directions set out above without any of them being scheduled.