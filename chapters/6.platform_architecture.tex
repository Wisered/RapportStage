...
This separation is not incidental. It extends to the rest of the application the same discipline that governs the engine's own design, and it rests on principle P4 (Section~\ref{chap:method}): the referential in \texttt{referential.\allowbreak yaml} is the single source of truth for the business rules, never duplicated in the database schema or in the frontend code.
...
\begin{table}[ht!]
  ...
  \begin{tabular}{@{}p{3.6cm}p{4.4cm}p{4.6cm}@{}}
    ...
    Centralized access control & \texttt{authz.\allowbreak py} & OWASP A01, Broken Access Control; IDOR \\
...
The referential's \texttt{ai\_\allowbreak module\_\allowbreak scope} lists four candidate models, \texttt{mistral:\allowbreak 7b-\allowbreak instruct}, \texttt{mixtral:\allowbreak 8x7b}, \texttt{llama3.\allowbreak 1:\allowbreak 8b} and \texttt{qwen2.\allowbreak 5:\allowbreak 7b}, all in the 7 to 8 billion parameter range. Ollama actually runs \texttt{qwen2.\allowbreak 5:\allowbreak 3b} by default. The arbitration criterion was latency, not quality in the abstract: the demo runs on CPU inside the VirtualBox VM described in Section~\ref{sec:platform-networking}, and a 7B model's generation time there makes the recommendations endpoint impractical, several minutes for a single request. \texttt{qwen2.\allowbreak 5:\allowbreak 3b} keeps generation to roughly a minute while still producing a usable narrative, because the task the model is actually given is constrained, summarising and enriching a handful of pre-supplied recommendations for the weak best practices, not open-ended reasoning from scratch. The model is one environment variable away from being swapped once a GPU or a larger machine is available; a proper benchmark across the candidate models remains future work rather than something this MVP needed to settle.
...
\begin{table}[ht!]
  ...
  \begin{tabular}{@{}llp{3.1cm}p{3.6cm}@{}}
    ...
    \texttt{mistral:\allowbreak 7b-\allowbreak instruct} & 7B & several minutes & Candidate, rejected on latency \\
    \texttt{mixtral:\allowbreak 8x7b} & 8x7B (MoE) & not benchmarked & Candidate \\
    \texttt{llama3.\allowbreak 1:\allowbreak 8b} & 8B & not benchmarked & Candidate \\
    \texttt{qwen2.\allowbreak 5:\allowbreak 7b} & 7B & not benchmarked & Candidate \\
    \bottomrule
  \end{tabular}
...
\begin{enumerate}
  \item read the new address with \texttt{ip addr show | grep "inet " | grep -\allowbreak v 127.\allowbreak 0.\allowbreak 0.\allowbreak 1};
  \item update \texttt{NEXT\_\allowbreak PUBLIC\_\allowbreak API\_\allowbreak URL} in \texttt{frontend/\allowbreak .\allowbreak env.\allowbreak local} to the new address;
  \item add the new address to \texttt{cors\_\allowbreak origins} in \texttt{backend/\allowbreak app/\allowbreak config.\allowbreak py};
