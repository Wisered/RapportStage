...
\ref{sec:validation-test-profiles}), and stands at 70 today. The backend
...
terms in \ref{sec:validation-ai-evaluation}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{5.6cm}p{9.4cm}@{}}
    ...
    \texttt{test\_\allowbreak bounds\_\allowbreak across\_\allowbreak sweep} & 20 parametrized combinations, proves the final score stays in $[0, 100]$ \\
    ...
    \texttt{test\_\allowbreak penalty\_\allowbreak threshold\_\allowbreak is\_\allowbreak strict\_\allowbreak greater\_\allowbreak than} & A gap exactly at the threshold does not penalize, just above does \\
    \bottomrule
  \end{tabular}
...
by any test (finding H1, \ref{chap:scoring-engine}). This is worth dwelling on briefly. A
...
than open-ended, it summarizes and enriches recommendations that the
...
larger model remains future work (\ref{chap:impact}).
...
\ref{chap:platform}. The other two sit downstream of generation, before any
text reaches the user. First, identifier normalization corrects the shapes a
small model
...
it reaches the user. Retrieval-augmented generation proper, also surveyed
...
(\ref{sec:validation-worked-example}).
...
(\ref{sec:validation-failure-modes}). A real but contextually irrelevant
