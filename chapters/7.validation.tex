...
authentication and access control, and now counts 141 tests. This figure
covers a wider perimeter than the engine suite above and should not be
merged with it, three of these 141 tests are dedicated specifically to a
privacy guarantee, that neither N/A justification text nor any personal
identifier ever reaches the AI prompt, which Section~\ref{sec:validation-failure-modes}
returns to.
...
test profile through the pipeline. The four phase scores it produces, 75
for Prevention, 68 for Preparedness, 75 for Response, and 36 for Recovery,
average to a raw global score of 63 under the referential's equal 25
percent phase weighting. BP08, a foundational best practice, scores 0 on
this profile, well below its 40-point threshold, which triggers its
foundational gate and caps the global score at 65. Because the raw score of
63 already sits below that cap, the gate is recorded as applied but does not
...
from 63 to a final global score of 48 (48.4 before rounding), mapped to
...
three hallucination-mitigation strategies surveyed in
...
it reaches the user. The third strategy surveyed there, retrieval-augmented
generation proper, is not used here, the anchor is a fixed list rather than
a retrieved one, since the sixteen best practices are few enough to inject
in full rather than search over.
