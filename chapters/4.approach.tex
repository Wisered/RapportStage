...
%   tnreport.cls ALREADY loads booktabs and tikz with the
%   positioning/arrows.meta/fit libraries, no need to add them again.
%   The only addition needed is the "backgrounds" tikz library, used
%   for the dashed stage boxes in the Section 4.2 figure:
%     \usetikzlibrary{backgrounds}
%   Also confirm \usepackage{seqsplit} is loaded somewhere in the
%   preamble (used here for long code identifiers such as file and
%   module names, so they can break across lines inside \texttt{}).
%
...
two documents. \texttt{\seqsplit{CLAUDE.md}} is the framing document for
...
\texttt{\seqsplit{ARCHITECTURE.md}} is an append-only, dated log of the
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}cp{0.82\textwidth}@{}}
    ...
    8 & Committing \texttt{\seqsplit{.env}} files or other secrets. \\
...
AssessmentResult; that result is then read, never written, by the local
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.32\textwidth}p{0.58\textwidth}@{}}
    ...
    P4 -- Single source of truth & \texttt{\seqsplit{referential.yaml}} is the only specification of the framework; no code ever hardcodes a question, a weight, or a rule. \\
...
OWASP lists as the most common web application weakness. The concrete
...
version of the same trade-off. The referential lists several 7B-class
candidates for the local model, \texttt{\seqsplit{mistral:7b-instruct}},
\texttt{\seqsplit{mixtral:8x7b}}, \texttt{\seqsplit{llama3.1:8b}}, and
\texttt{\seqsplit{qwen2.5:7b}}, none of which is what the platform actually
runs: the model deployed is \texttt{\seqsplit{qwen2.5:3b}}, smaller than any
of them. The choice was driven by latency rather than by a formal
comparison of output quality, which \texttt{\seqsplit{CLAUDE.md}} itself
defers to a later sprint. On the CPU-bound hardware used for the demo, a 7B model pushed a
single recommendation request into multiple minutes, impractical for a
live, client-facing tool, while the 3B model keeps generation to about a
minute. The resulting quality loss was judged acceptable because the AI's
task stays narrow, summarizing and enriching a fixed set of referential
recommendations for a short list of weak best practices, rather than
...
same freshly computed \texttt{\seqsplit{AssessmentResult}} as the score
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.3\textwidth}p{0.32\textwidth}p{0.18\textwidth}@{}}
    ...
    Single \texttt{\seqsplit{scoring\_bridge.py}} translation seam & Scoring logic duplicated across API routes & P1 \\
    \bottomrule
  \end{tabular}
