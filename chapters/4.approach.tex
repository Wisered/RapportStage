...
The chapter is organised in four parts. Section~\ref{sec:working-method}
...
The project was organised as a sequence of bounded sprints, each defined by
...
instead of declaring the work finished by feel; where the work actually
departed from the plan, Chapter~\ref{chap:pm} records the departure rather
than the plan.
...
reproduced in Table~\ref{tab:anti-patterns} (Appendix~\ref{app:tables}) so that later chapters can
...
that double as an executable specification of the scoring rules, and it went through an audit that fed corrections back into the code before any backend work began. 
The audit was carried out by a
separate, freshly initialised review with no prior exposure to the
...
revisited afterward. Table~\ref{tab:principles-summary}, in Appendix~\ref{app:tables}, summarises them;
...
augmentation; the engine's internals and the independent audit it went
through are described in Chapter~\ref{chap:scoring-engine}.
...
organisations toward demonstrable sovereignty over the data they process,
and an assessment produced by this platform is, in effect, a detailed map
of an organisation's cyber weaknesses. Sending that map to a third-party
API, however reputable, would undermine the very trust the platform is
meant to build. Because inference stays local, no separate anonymisation
...
structured account of an organisation's crisis-management gaps, drives
...
volume is encrypted at rest, free-text fields such as Not Applicable (N/A) justifications
...
referential text, but never an organisation name, an identifier, or the
content of a justification field. Access control
follows the same logic, centralised in a single module rather than
...
auditable artefact: reviewing what the platform actually measures means
...
The data model has included users, organisations, roles, and the
...
Table~\ref{tab:rejected-alternatives} summarises the three below.
...
That gap in capability is real, and Chapter~\ref{chap:validation} returns to
it directly. It was set aside anyway, for the reason given in
Section~\ref{principle:local-first}: the sensitivity of assessment data rules
out sending it to a third party regardless of the quality gained. The choice of 
local inference over a cloud API was therefore imposed by the platform's subject 
matter rather than decided purely on technical merit.
...
It was chosen over \texttt{qwen2.5:\allowbreak 7b}, the same model one size up,
on generation times measured on the project's own hardware; comparing two sizes
of one family holds everything but the parameter count constant, and
Chapter~\ref{chap:validation} reports the figures and the limits of that
comparison. A systematic benchmark across all four candidates was never run, and
\texttt{CLAUDE.md} itself defers it to a later sprint. The resulting quality loss was judged
acceptable because the AI's task stays narrow, summarising and enriching a
...
Ollama. They were rejected in favour of a direct, thin HTTP client that
posts to Ollama's own generation endpoint and normalises every transport
...
it, since that is what keeps the prompt's no personally identifiable information (PII) guarantee verifiable;
...
directly. That path was rejected in favour of a single
